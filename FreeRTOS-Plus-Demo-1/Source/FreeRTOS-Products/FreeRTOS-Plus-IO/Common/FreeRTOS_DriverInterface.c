...
//
//
//lecture - i2c-eeprom - refer to LPCXpresso17xx-base-board.h for a table of
//          supported devices for this controller/board combination - typically,
//          a bsp information !!!
//
//lecture - i2c-eeprom - refer to FreeRTOS_DriverInterface.h for more details
//          on objects and tables used !!!
//
//lecture - UART - most points are same as that of I2C example
//
/*-----------------------------------------------------------*/
...
/*-----------------------------------------------------------*/
//
//lecture - i2c2-eeprom - this api is invoked,when i2c2 master must be opened
//
//lecture - i2c2-eeprom - second parameter is ignored
//
//lecture - i2c-eeprom - extract the table entry based on the device name provided
//
//lecture - i2c-eeprom - assuming that the device name is valid, create an
//                       an active device/control object - heap is used
//                       to allocate the memory for the control object !!!
//
//lecture - UART - this api is used to open UART3 to be used as command console
//                 most comments applicable for I2C are applicable here as well
//
//
		//lecture - UART - initialize the specific device instance no.
		//                 in this case, it will 3 from UART3
		//
		//lecture - UART - initialize the appropriate fields
		//
		//lecture - UART - invoke boardFreeRTOS_PopulateFunctionPointers()
		//                       as per peripheral type and driver
		//                       - refer to LPCXpresso17xx-base-board.h -
		//                       invokes vFreeRTOS_lpc17xx_PopulateFunctionPointers
		//
		//lecture - UART  - refer to FreeRTOS_lpc17xx_DriverInterface.c
		//                       for vFreeRTOS_lpc17xx_PopulateFunctionPointers()
		//
		//lecture - UART  - if successful, return pointer to the active device /control object -
		//                       in this case, pointer will be to the UART3's active device object
		//
		//
...
/*-----------------------------------------------------------------------------------------------------------------------
 * we rise the code to privilege level as we need to access this API in unprivileged task also . . .
 *
 ------------------------------------------------------------------------------------------------------------------------*/
...
//
//
//lecture - i2c-eeprom - invoked from from I2C-coordinator.c and related files -
//                       used to configure the peripheral's tx and rx modes, and
//                       other operations !!!
//        - i2c-eeprom - need to dig deep into the related files - certain other control objects
//                       are created and initialized - transfer control structures are being
//                       allocated and initialized !!!
//
//lecture - UART - invoked from command console code UART-interrupt-driven-command-console.c -
//                 most points are similar to that of i2c case
//
/* See the function prototype definition for documentation information. */
portBASE_TYPE FreeRTOSGenericioctl( Peripheral_Descriptor_t const xPeripheral, uint32_t ulRequest, void *pvValue )
{
	...
	switch( ulRequest )
	{
		...
		case ioctlUSE_ZERO_COPY_TX	:
			...
			#if ioconfigUSE_ZERO_COPY_TX == 1
			{
				...
				//
		        //lecture - i2c-eeprom - refer to IOUtils_ZeroCopyTx.c for
				//                       xIOUtilsConfigureZeroCopyTx() method !!!
				//
                //lecture - UART -  refer to IOUtils_ZeroCopyTx.c for
				//                  xIOUtilsConfigureZeroCopyTx() method !!!
				//
				//
		...
		case ioctlUSE_CIRCULAR_BUFFER_RX :
			...
			{
				//
				//lecture - i2c-eeprom - refer to IOUtils_CircularBufferRx.c
				//
				//
				//lecture - UART -  refer to IOUtils_CircularBufferRx.c for
				//                  xIOUtilsConfigureCircularBufferRx() method !!!
				//
			    //
		...
		case ioctlUSE_CHARACTER_QUEUE_RX	:
			...
			break;
//aaa-zzz  : Task notication based TX and RX modes : utilising task notifcation feature
		case ioctlUSE_TASKNOTIFICATION_TX	:
		case ioctlUSE_TASKNOTIFICATION_RX	:
			#if ( ioconfigUSE_TASK_NOTIFICATION_TX == 1 ) || ( ioconfigUSE_TASK_NOTIFICATION_RX == 1 )
			{
				/* The peripheral is going to use a Tasknotification_Circular_Buffer_State_t
				structure for reception.  This places received characters into
				a circular buffer, then allows a pointer to the buffer to be returned
				directly in a read function. */
				xReturn = xIOUtilsConfigureTasknotificationCircularBuffer( pxPeripheralControl,ulRequest, ( portBASE_TYPE ) pvValue );

				/* Tasknotification based Circular buffer Tx/Rx can only be used when interrupts are also
				used.  Enabling interrupts is a peripheral specific operation. */
				ulRequest = ioctlUSE_INTERRUPTS;
				pvValue = ( void * ) pdTRUE;
				xCommandIsDeviceSpecific = pdTRUE;
			}
			#endif /*  ioconfigUSE_TASK_NOTIFICATION_TX == 1 ) || ( ioconfigUSE_TASK_NOTIFICATION_TX == 1  */
			break;
		...
		case ioctlSET_RX_TIMEOUT	:
			...
			break;
        //
        //lecture - i2c-eeprom - even if we request for mutex operations from eeprom test code
		//                       if the current tx mode is not zero copy tx, the operations are
		//                       not invoked (??)
		//
		...
		case ioctlCLEAR_RX_BUFFER :
			...
			else if( pxPeripheralControl->pxRxControl->ucType == ioctlUSE_TASKNOTIFICATION_RX )  //aaa-zzz
			{
				#if ioconfigUSE_TASK_NOTIFICATION_RX == 1
				{
					vIOUtilsClearTasknotificationRxCircularBuffer( pxPeripheralControl );
					xReturn = pdPASS;
				}
				#endif /* ioconfigUSE_TASK_NOTIFICATION_RX*/
			}
			else
			{
    ...
    //
	//lecture - i2c-eeprom - in our case, this is the place where hw interrupts
	//                       are configured !!
	//                       refer to FreeRTOS_lpc17xx_i2c.c and look into
	//                       FreeRTOS_I2C_ioctl() method
	//
	//lecture - i2c-eeprom - we will reach here for the following i2c specific
	//                       ioctl commands
	//
	//          i2c-eeprom - FreeRTOS_ioctl( xI2CPort, ioctlSET_SPEED, i2cEEPROM_CLOCK );
	//                       from i2c-to-and-from-eeprom.c and also for zero copy tx
	//                       mode interrupt setting
	//
	//          i2c-eeprom - refer to FreeRTOS_lpc17xx_i2c.c for i2c specific ioctl
	//                       operations - refer to FreeRTOS_I2C_ioctl()
	//
	//
	//lecture - UART - peripheral specific ioctl method is invoked
	//
