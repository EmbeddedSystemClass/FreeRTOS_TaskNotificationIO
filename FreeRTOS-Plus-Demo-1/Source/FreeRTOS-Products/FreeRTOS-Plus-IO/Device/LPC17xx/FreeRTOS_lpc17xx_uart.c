...
static inline size_t prvFillFifoFromBuffer( LPC_USART_T * const pxUART, uint8_t **ppucBuffer, const size_t xTotalBytes ); //aaa-zzz
...
const uint8_t cPeripheralNumber = diGET_PERIPHERAL_NUMBER( pxPeripheralControl );
	...
	if( cPeripheralNumber < boardNUM_UARTS )
	{
		...
		//	boardCONFIGURE_UART_PINS( cPeripheralNumber, xPinConfig );
			...
			//UART1 --P0_15,P0_16
			//p0_15/TXD1 ---(jp3)----Tx1---Tx1_out---J5
			//p0_16/RXD1 ---(jp2)---Rx1---Rx1_out---J5
            if(cPeripheralNumber == 1)
            {
			    Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 15, IOCON_MODE_INACT | IOCON_FUNC2); //aaa-zzz
			    Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 16, IOCON_MODE_INACT | IOCON_FUNC2); //aaa-zzz
            }
            //UART3 --P0_0,P0_1
            			//p0_0/TXD3 ---()----Tx3---Tx3_out---J4
            			//p0_1/RXD3 ---()---Rx3---Rx3_out---J4
			if(cPeripheralNumber == 3)
			{
				Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 0, IOCON_MODE_INACT | IOCON_FUNC2); //aaa-zzz
				Chip_IOCON_PinMuxSet(LPC_IOCON, 0, 1, IOCON_MODE_INACT | IOCON_FUNC2); //aaa-zzz
			}


			/* above is not generic - It is specific to UART1 . improove- more generic call
			 * which can initialise any UART */
...
//
	...
	//		xReturn = UART_Send( pxUART, ( uint8_t * ) pvBuffer, ( size_t ) xBytes, BLOCKING );
			xReturn = Chip_UART_SendBlocking( pxUART,  pvBuffer, ( int ) xBytes); //aaa-zzz
			...
			//aaa-zzz : replace this as well after understanding purpose
...
*/
			...
			case ioctlUSE_TASKNOTIFICATION_TX:
					...
					#if ioconfigUSE_UART_TASKNOTIFICATION_TX == 1
					{
						/* is There anything ??? to prevent multiple tasks attempting to
						read the circular buffer at any one time. */
	                    /*The implementation uses : task notifications to signal whenever new
						data is available in circular buffer */
#if 0
						ioutilsTRANSMIT_TO_TASKNOTIFICATION_CIRCULAR_BUFFER
						(
							pxPeripheralControl,
...
#endif
						xReturn=ioutilsTRANSMIT_TO_TASKNOTIFICATION_CIRCULAR_BUFFER(pxPeripheralControl,( ( uint8_t * ) pvBuffer ), xBytes); //aaa-zzz
					}
					#endif /* ioconfigUSE_UART_TASKNOTIFICATION_TX */
					break;
...
//
	if( diGET_RX_TRANSFER_STRUCT( pxPeripheralControl ) == NULL )
	{
		#if ioconfigUSE_UART_POLLED_RX == 1
		{
			...
			xReturn = Chip_UART_Read( pxUART, pvBuffer, xBytes); //aaa-zzz
		}
...
*/
			...
			case ioctlUSE_TASKNOTIFICATION_RX	:
					...
					#if ioconfigUSE_UART_TASKNOTIFICATION_RX == 1
					{
						/* is There anything ??? to prevent multiple tasks attempting to
						read the circular buffer at any one time. */
	                    /*The implementation uses : task notifications to signal whenever new
						...
						ioutilsRECEIVE_FROM_TASKNOTIFICATION_CIRCULAR_BUFFER
						(
							pxPeripheralControl,
							Chip_UART_IntDisable(pxUART, UART_IER_RBRINT),	/* Disable peripheral. */
							Chip_UART_IntEnable( pxUART, UART_IER_RBRINT), 	/* Enable peripheral. */
					...
					#endif /* ioconfigUSE_UART_TASKNOTIFICATION_RX */
					break;
...
{
Peripheral_Control_t * const pxPeripheralControl = ( Peripheral_Control_t * const ) pxPeripheral;
//UART_CFG_Type xUARTConfig;  //aaa-zzz
...
*/
					//Chip_UART_IntDisable(pxUART, UART_IER_RBRINT); // needed ?? -aaa-zzz
					Chip_UART_IntEnable( pxUART, UART_IER_THREINT);
				//	Chip_UART_IntEnable( pxUART, UART_IER_RBRINT | UART_IER_RLSINT);
...
//	ulInterruptSource = UART_GetIntId( LPC_UART3 );
    ...
    ulInterruptSource = ( LPC_UART3->IIR & 0x03CF); //aaa-zzz
		...
		if( pxTransferStruct != NULL )
		{
			switch( diGET_TRANSFER_TYPE_FROM_CONTROL_STRUCT( pxTransferStruct ) )
			{
        		...
        		case ioctlUSE_TASKNOTIFICATION_RX :
					...
					{
#if 1
         				ioutilsRX_CHARS_INTO_TASKNOTIFICATION_CIRCULAR_BUFFER_FROM_ISR(
							pxTransferStruct, 	/* The structure that contains the reference to the circular buffer. */
							( ( LPC_UART3->LSR & UART_LSR_RDR ) != 0 ), 	/* While loop condition. */
							LPC_UART3->RBR,			/* Register holding the received character. */
							ulReceived,
...
#endif
         				//ulReceived = ioutilsRX_CHARS_INTO_TASKNOTIFICATION_CIRCULAR_BUFFER_FROM_ISR(pxTransferStruct,&xHigherPriorityTaskWoken);
					}
					#endif /* ioconfigUSE_UART_TASKNOTIFICATION_RX */
					break;
		...
		if( pxTransferStruct != NULL )
		{
			switch( diGET_TRANSFER_TYPE_FROM_CONTROL_STRUCT( pxTransferStruct ) )
			{
				...
				case ioctlUSE_TASKNOTIFICATION_TX :
					#if ioconfigUSE_UART_TASKNOTIFICATION_TX == 1
					{
         				ioutilsTX_CHARS_OF_TASKNOTIFICATION_CIRCULAR_BUFFER_FROM_ISR(
							pxTransferStruct, 	/* The structure that contains the reference to the circular buffer. */
							( UART_FIFOLVL_TXFIFOLVL( LPC_UART3->FIFOLVL ) != ( UART_TX_FIFO_SIZE - 1 ) ), 	/* While loop condition. */
							( LPC_UART3->THR = ucChar ),			/* Register holding the received character. */
							ulReceived,
							xHigherPriorityTaskWoken
						);
					}
