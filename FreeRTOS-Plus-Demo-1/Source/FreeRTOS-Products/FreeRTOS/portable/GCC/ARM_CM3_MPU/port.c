...
//eripheral_Descriptor_t FreeRTOS_Generic_open( const int8_t *pcPath, const uint32_t ulFlags );
Peripheral_Descriptor_t MPU_FreeRTOS_open( const int8_t *pcPath, const uint32_t ulFlags );

size_t MPU_FreeRTOS_write( Peripheral_Descriptor_t const xPeripheral, const void *pvBuffer, const size_t xBytes );
//size_t FreeRTOS_Generic_write( Peripheral_Descriptor_t const xPeripheral, const void *pvBuffer, const size_t xBytes );

size_t MPU_FreeRTOS_read( Peripheral_Descriptor_t const xPeripheral, const void *pvBuffer, const size_t xBytes );
//size_t FreeRTOS_Generic_read( Peripheral_Descriptor_t const xPeripheral, const void *pvBuffer, const size_t xBytes );
...
extern uint32_t __SRAM_segment_end__[];
	...
	if( *portMPU_TYPE == portEXPECTED_MPU_TYPE_VALUE )
	{
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_PRIVILEGED_READ_WRITE ) |
		...
		/* Setup the whole SRAM in PRIVILEGED mode. This where the main code and handler stack is stored */
		*portMPU_REGION_BASE_ADDRESS = 	( ( uint32_t ) __SRAM_segment_start__) |
									   	( portMPU_REGION_VALID) |
									   	( portSRAM_REGION );
		...
		*portMPU_REGION_ATTRIBUTE =		( portMPU_REGION_PRIVILEGED_READ_WRITE | portMPU_REGION_EXECUTE_NEVER ) |
...
/*  aaa-zzz
 *
 * MRS{cond} Rd, spec_reg :  move from special-reg --> reg
 *TST{cond} Rn, Operand2  : Test bits and update  conditional falgs
 *
 *  IT<x><y><z><cond>  --- IF then  else instr
 *
 *  x,y,z are optional-- must be either T (for "then") or E (for "else").
 *  <cond> is any of  NE or EQ or GT, etc. that are reflected in the APSR flags
 *
 *  For each T and each E after IT, you must have a subsequent instruction
 *  in the same order that matches up.
 *
 *  Each matching subsequent instruction must have conditions that match up with the IT instruction.
 *
 *  itte ne -- if then-then-else  Not-Equal
 *  CONTROL[0]!=0 then  movne r0, #0  => return false
 *  CONTROL[0]!=0 then  svcne %0      ==> svc 2{%0=> first i/p arg=> portSVC_RAISE_PRIVILEGE }
 *  else  moveq r0, #1 {==>  CONTROL[0]==0  } => return true => already in priv access level
 *
...
BaseType_t xRunningPrivileged = prvRaisePrivilege();
	...
	xReturn = FreeRTOSGenericopen( *pcPath, ulFlags );
...
#define FreeRTOS_Generic_write( xPeripheral, pvBuffer, xBytes  ) ( ( Peripheral_Control_t * ) xPeripheral )->write( ( ( Peripheral_Control_t * ) xPeripheral ), ( pvBuffer ), ( xBytes ) )

	xReturn = FreeRTOS_Generic_write( xPeripheral, pvBuffer, xBytes  );
...
#define FreeRTOS_Generic_read( xPeripheral, pvBuffer, xBytes  ) ( ( Peripheral_Control_t * ) xPeripheral )->read( ( ( Peripheral_Control_t * ) xPeripheral ), ( pvBuffer ), ( xBytes ) )

	xReturn = FreeRTOS_Generic_read( xPeripheral, pvBuffer, xBytes  );
...
BaseType_t xRunningPrivileged = prvRaisePrivilege();
		...
		xReturn = FreeRTOSGenericioctl( xPeripheral, ulRequest, pvValue );
...
/*
 *   ?? - check is it asking for MPU_ulTaskNotifyGive ?? why --we are using only MPU_ulTaskNotifyGivefromISR
uint32_t MPU_ulTaskNotifyTake( BaseType_t xClearCountOnExit, TickType_t xTicksToWait )
{
size_t xReturn;
BaseType_t xRunningPrivileged = prvRaisePrivilege();
...
portBASE_TYPE MPU_xTimerGenericCommand( TimerHandle_t xTimer, const BaseType_t xCommandID, const TickType_t xOptionalValue, BaseType_t * const pxHigherPriorityTaskWoken, const TickType_t xTicksToWait )
{
size_t xReturn;
BaseType_t xRunningPrivileged = prvRaisePrivilege();
