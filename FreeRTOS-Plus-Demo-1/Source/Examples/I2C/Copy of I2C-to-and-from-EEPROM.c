...
//int i2c_id = pxpheri_ctl->cPeripheralNumber;
    ...
    //
// set to polling mode
  //   i2c_set_mode(i2c_id, 1);  //aaa-zzz
// better @ polling case of freertos_i2c_read
	...
	for( ulPage = 0UL; ulPage < ( i2cTOTAL_EEPROM_SIZE / i2cPAGE_SIZE ); ulPage++ )
	{
		/* Ensure t+he data buffer does not contain 0x00 already. */
