...
#define uartst1_TEST_TRANSACTIONS			( 1 )
...
//aaa-zzz
//#define uartstasknotifCIRCULAR_BUFFER_SIZE			( ( void * ) ( strlen( ( char * ) pcTestMessage3 ) * ( size_t ) 2U ) )
...
//aaa-zzz
static const int8_t * const pcRTestMessage1 = ( int8_t * ) "asdf";
static const int8_t * const pcRTestMessage2 = ( int8_t * ) "1234";
static const int8_t * const pcRTestMessage3 = ( int8_t * ) "xyz";
static const int8_t * const pcRTestMessage4 = ( int8_t * ) "qwerty";
...
portBASE_TYPE xReturned;
	...
	xReturned = xTaskCreate( prvUARTTxOperationModesTestTask,		/* The task that demonstrates the various modes in which the UART can transmit. */
							...
							configMAX_PRIORITIES - 2 | portPRIVILEGE_BIT,		//aaa-zzz		/* The priority allocated to the task.  This needs to be below the Rx task priority, otherwise the polling tests won't work. */
	...
	xReturned = xTaskCreate( prvUARTRxOperationModesTestTask,		/* The task that demonstrates the various modes in which the UART can receive. */
							...
							configMINIMAL_STACK_SIZE * 2 | portPRIVILEGE_BIT,	//aaa-zzz		/* The size of the stack allocated to the task. */
							NULL,									/* The parameter is not used, so NULL is passed. */
...
//	xReturned = xTaskIsTaskSuspended( xRxTaskHandle );
	...
	if (eTaskGetState(xRxTaskHandle) == eSuspended )   //aaa-zzz
		xReturned = 1;
	prvCheckTestResult( xReturned, __LINE__ );
	...
	for( ;; )
	{
		...
		while( eTaskGetState( xRxTaskHandle ) != eSuspended )   //aaa-zzz
		{
...
portTickType xTimeBefore, xTimeAfter;
...
#if 1
	xStringLength = strlen( ( char * ) pcRTestMessage1 );
	for( xTestLoops = 0; xTestLoops < uartst1_TEST_TRANSACTIONS; xTestLoops++ )
	{
		/* Clear the Rx buffer, then poll for characters one at a time, placing
		them in the Rx buffer. */
		memset( pcRxBuffer, 0x00, xRxBufferSize );
		...
		/* The whole string should now have been received.  Is it correct? */
		xReturned = strcmp( ( char * ) pcRxBuffer, ( char * ) pcRTestMessage1 );
		prvCheckTestResult( ( xReturned == 0 ), __LINE__ );
	}
#endif
	...
	for( ;; )
	{
...
#if 1
		xStringLength = strlen( ( char * ) pcRTestMessage2 );
		for( xTestLoops = 0; xTestLoops < uartst1_TEST_TRANSACTIONS; xTestLoops++ )
		{
			/* Clear the Rx buffer. */
			memset( pcRxBuffer, 0x00, xRxBufferSize );
			...
			/* The whole string should now have been received.  Is it
			correct? */
			xReturned = strcmp( ( char * ) pcRxBuffer, ( char * ) pcRTestMessage2 );
			prvCheckTestResult( ( xReturned == 0 ), __LINE__ );
		}
#endif
		...
		xStringLength = strlen( ( char * ) pcRTestMessage3 );
		for( xTestLoops = 0; xTestLoops < uartst1_TEST_TRANSACTIONS; xTestLoops++ )
		{
			/* Clear the Rx buffer. */
			memset( pcRxBuffer, 0x00, xRxBufferSize );
			...
			/* The whole string should now have been received.  Is it correct? */
			xReturned = strcmp( ( char * ) pcRxBuffer, ( char * ) pcRTestMessage3 );
			prvCheckTestResult( ( xReturned == 0 ), __LINE__ );
		}
