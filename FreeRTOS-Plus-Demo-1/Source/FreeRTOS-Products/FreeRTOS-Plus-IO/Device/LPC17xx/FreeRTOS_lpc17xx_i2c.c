...
task.h is included from an application file  ????? why
 */
...
//#include "lpc17xx_pinsel.h"     //aaa-zzz
...
//
//
//lecture - i2c-eeprom - in our case, i2c2 master is used and respective
//                       objects and information will be used
//
//lecture - i2c-eeprom - we will be using the below mentioned objects and
//          information
//
//lecture - refer to lpc17xx user-manual for the no. of i2c masters available
//          refer to the schematic and EA-baseboard for the no. of available
//          masters on the board - meaning, that can be availed ???
//
//
...
//
//
//lecture - i2c-eeprom - refer to LPC17xx.h of cmsis core library - irq nos.
//                       can be extracted from LPC17xx.h !!!
//          i2c-eeprom - refer to ch6 / table 50 of lpc17xx user-manual
//
/* Stores the IRQ numbers of the supported I2C ports. */
static const IRQn_Type xIRQ[ boardNUM_I2CS ] = { I2C0_IRQn, I2C1_IRQn, I2C2_IRQn };
//
//
//lecture - i2c-eeprom - in our case, refer to I2C-to-and-from-EEPROM.c -
//          i2c-eeprom - slave address is set-up and passed for the
//                       eeprom device - refer to the comments, in that file
//
//
...
//
//
//lecture - i2c-eeprom - many of the freertos+io objects are defined in FreeRTOS_DriverInterface.h
//
//
//lecture - i2c-eeprom - some of the objects mentioned here can be found in lpc17xx_i2c.h
//                       of cmsis firmware driver library - refer to it as needed
//lecture - i2c-eeprom - some of the objects mentioned here can be found in LPC17xx.h of
//                       cmsis core library - refer to it as needed
//
//lecture - i2c-eeprom - refer to FreeRTOS_IOUtils.c for more details and code - set up
//                       of transfer control structure !!!
//
//lecture - i2c-eeprom - if the basic sw set-up is completed, hw initialization is done
//                       to activate the master i2c controller - see the comments below !!!
//
/*-----------------------------------------------------------*/
...
//
//lecture - i2c-eeprom - the block of addresses associated with I2C2 - can be
//                       used to access respective registers of I2C2 master
//
//
...
I2C_XFER_T  *pxI2CTxTransferDefinition = NULL, *pxI2CRxTransferDefinition = NULL;
	...
	if( cPeripheralNumber < boardNUM_I2CS )
	{
		...
		polled mode. */
		//
		//lecture - i2c-eeprom - refer to cmsis firmware library header for i2c for more
		//          information and details !!!
		//
		//
		pxI2CTxTransferDefinition = (  I2C_XFER_T * ) pvPortMalloc( sizeof(  I2C_XFER_T ) );
		pxI2CRxTransferDefinition = (  I2C_XFER_T * ) pvPortMalloc( sizeof(  I2C_XFER_T ) );
		...
		if( ( pxI2CTxTransferDefinition != NULL ) && ( pxI2CRxTransferDefinition != NULL ) )
		{
            ...
            //lecture - i2c-eeprom - actually, initially initialize the active
			//                       device object/device control object
			//                       default is polled mode tx and polled mode rx
			//
			if( ( pxPeripheralControl->pxTxControl != NULL ) && ( pxPeripheralControl->pxRxControl != NULL ) )
			{
                ...
                //
				//
				//lecture - i2c-eeprom - why is there a critical section used here ??
				//                       possibly, this is where the hw access is done !!!
				//
				//lecture - i2c-eeprom - refer to LPCXpresso17xx-base-board.h and
				//                       also firmware driver files - in addition,
				//                       refer to lpc17xx user-manual table 80 and
				//                       other i2c(2) related sections of chapter 19
				//
				//lecture - i2c-eeprom - refer to lpc17xx_i2c.c and
				//                       lpc17xx_pinsel.c of firmware driver lib
				//
...
*/
					...
					Chip_I2C_SetMasterEventHandler(cPeripheralNumber, Chip_I2C_EventHandlerPolling);
...
//
//
//lecture - i2c-eeprom - invoked i2c-to-and-from-eeprom.c - gets the transfer structure and gets the
//                       transfer mode for tx - accordingly, takes action !!!
//          i2c-eeprom - refer to FreeRTOS_DriverInterface.h for control objects and macros
//
//
//
...
LPC_I2C_T * const pxI2C = ( LPC_I2C_T * const ) diGET_PERIPHERAL_BASE_ADDRESS( pxPeripheralControl );
I2C_XFER_T *pxI2CTransferDefinition;
const int8_t cPeripheralNumber = diGET_PERIPHERAL_NUMBER( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
	...
	switch( diGET_TX_TRANSFER_TYPE( pxPeripheralControl ) )
	{
	   //
	   //
	   //lecture - i2c-eeprom - for tx polling mode call, this case is used - refer to
	   //          i2c-eeprom - refer to for I2C_M_SETUP_Type object  lpc17xx_i2c.h
	   //
	   //lecture - i2c-eeprom - application buffer is being passed to the transfer control object
	   //                       from here, hw issues are taken care ??
	   //lecture - i2c-eeprom - refer to lpc17xx_i2c.c firmware driver - in addition,
	   //                       refer to chapter 19 of lpc17xx user-manual ??
	   //
	   //
		case ioctlUSE_POLLED_TX :
			...
			#if ioconfigUSE_I2C_POLLED_TX == 1
			{
				...
				pxI2CTransferDefinition = ( I2C_XFER_T * ) diGET_TX_TRANSFER_STATE( pxPeripheralControl );
...
//aaa-zzz debug here  i2c-busy
				...
				*/
				...
				{
				   uint8_t ret_len;
				   ret_len=Chip_I2C_MasterSend(cPeripheralNumber, ucSlaveAddresses[ cPeripheralNumber ], pvBuffer,xBytes);
				   xReturn = ret_len;
                }
				//
		...
		//
	    //
		//lecture - i2c-eeprom - this is invoked, when the zero copy Tx mode is selected
		//          i2c-eeprom - refer to  IOUtils_ZeroCopyTx.c and IOUtils_ZeroCopyTx.h
		//
		//
...
//				pxI2C->I2CONCLR = ( I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC | I2C_I2CONCLR_AAC | I2C_I2CONCLR_STAC ); //aaa-zzz
				pxI2C->CONCLR = ( I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC | I2C_I2CONCLR_AAC | I2C_I2CONCLR_STAC );

				/* Set start flag. */
				//pxI2C->I2CONSET = I2C_I2CONSET_STA; //aaa-zzz
				pxI2C->CONSET = I2C_I2CONSET_STA;


				/* Now enable interrupts. */
				//I2C_IntCmd( pxI2C, ENABLE );
				NVIC_EnableIRQ(xIRQ[cPeripheralNumber]); //aaa-zzz
			}
...
size_t xReturn = 0U;
//LPC_I2C_TypeDef * const pxI2C = ( LPC_I2C_TypeDef * const ) diGET_PERIPHERAL_BASE_ADDRESS( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
LPC_I2C_T * const pxI2C = (  LPC_I2C_T * const ) diGET_PERIPHERAL_BASE_ADDRESS( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
I2C_XFER_T  *pxI2CTransferDefinition;
const int8_t cPeripheralNumber = diGET_PERIPHERAL_NUMBER( ( ( Peripheral_Control_t * const ) pxPeripheral ) );
	...
	switch( diGET_RX_TRANSFER_TYPE( pxPeripheralControl ) )
	{
		case ioctlUSE_POLLED_RX :
            //
			//lecture - i2c-eeprom - refer to polled tx mode above and relevant comments
			//          i2c-eeprom - most of the comments apply with minor adjustments
			//
			//
			#if ioconfigUSE_I2C_POLLED_RX == 1
			{
				...
				here, so the application must ensure only one task attempts to
				make a polling write at a time. */
				pxI2CTransferDefinition = (  I2C_XFER_T * ) diGET_RX_TRANSFER_STATE( pxPeripheralControl );
				configASSERT( pxI2CTransferDefinition );
/*
				pxI2CTransferDefinition->slaveAddr = ucSlaveAddresses[ cPeripheralNumber ];
...
*/
				{
				    int ret;
					ret=Chip_I2C_MasterRead(cPeripheralNumber, ucSlaveAddresses[ cPeripheralNumber ], pvBuffer, xBytes);
					xReturn = ret;
				}
...
//					pxI2C->I2CONSET = I2C_I2CONSET_STA; //aaa-zzz
					...
					//I2C_IntCmd( pxI2C, ENABLE );  //aaa-zzz
				...
				ioutilsRECEIVE_CHARS_FROM_CIRCULAR_BUFFER
					(
						pxPeripheralControl,
						NVIC_DisableIRQ(xIRQ[cPeripheralNumber]),	/* Disable peripheral. */
						NVIC_EnableIRQ(xIRQ[cPeripheralNumber]), 	/* Enable peripheral. */
						( ( uint8_t * ) pvBuffer ),		/* Data destination. */
						xBytes,							/* Bytes to read. */
						xReturn							/* Number of bytes read.*/
					);
...
//
//
//lecture - i2c-eeprom - many of the basics are same as FreeRTOS_I2C_open() - refer to
//                       the comments provided above (??)
//          i2c-eeprom - several i2c specific ioctl commands are processed here (??)
//
//
//
		...
		switch( ulRequest )
		{
			...
			case ioctlUSE_INTERRUPTS :
				...
				//	I2C_IntCmd( pxI2C, DISABLE );
					NVIC_DisableIRQ( xIRQ[ cPeripheralNumber ] ); //aaa-zzz

					/* Clear any pending interrupts in the peripheral and
					NVIC. */
					//pxI2C->I2CONCLR = I2C_I2CONCLR_SIC; //aaa-zzz
					pxI2C->CONCLR = I2C_I2CONCLR_SIC;
			...
			case ioctlSET_SPEED :
				//
				//
                //lecture - i2c-eeprom - this is invoked in vI2C_EEPROMTest() from
				//                       i2c-to-and-from-eeprom.c
				//
				//lecture - i2c-eeprom - refer to  lpc17xx_i2c.c for more details and code
				//                       refer to lpc17xx user-manual for i2c master specific details
				//
				/* Set up the default I2C configuration. */

				 //aaa-zzz - Start
				//I2C_SetClock( pxI2C, ulValue );
				Chip_I2C_SetClockRate(cPeripheralNumber, ulValue);
...
//
//
//lecture - i2c-eeprom - connect the above write and read methods with this handler
//
//
//
...
Transfer_Control_t *pxTransferStruct;
	...
	//ulI2CStatus = ( LPC_I2C2->I2STAT & I2C_STAT_CODE_BITMASK );
      ulI2CStatus = ( LPC_I2C2->STAT & I2C_STAT_CODE_BITMASK );
	...
	switch( ulI2CStatus )
	{
		case I2C_I2STAT_NO_INF	:
			...
			interrupt. */
			//LPC_I2C2->I2CONCLR = I2C_I2CONCLR_SIC; //aaa-zzz
			LPC_I2C2->CONCLR = I2C_I2CONCLR_SIC;
			break;
...
//			LPC_I2C2->I2DAT = ( ucSlaveAddresses[ uxI2CNumber ] << 1U ) | ( uint8_t ) xDataDirection[ uxI2CNumber ]; //aaa-zzz
			LPC_I2C2->DAT = ( ucSlaveAddresses[ uxI2CNumber ] << 1U ) | ( uint8_t ) xDataDirection[ uxI2CNumber ];

			/* Clear the interrupt and the start bit. */
			//LPC_I2C2->I2CONCLR = I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC;  //aaa-zzz
			LPC_I2C2->CONCLR = I2C_I2CONCLR_SIC | I2C_I2CONCLR_STAC;
			break;
	...
	if( pxTransferStruct != NULL )
	{
		switch( ulI2CStatus )
		{
			...
			case I2C_I2STAT_M_TX_DAT_ACK:
				...
				//iouitlsTX_SINGLE_CHAR_FROM_ZERO_COPY_BUFFER_FROM_ISR( pxTransferStruct, ( LPC_I2C2->I2DAT = ucChar ), ulChar );//aaa-zzz
				iouitlsTX_SINGLE_CHAR_FROM_ZERO_COPY_BUFFER_FROM_ISR( pxTransferStruct, ( LPC_I2C2->DAT = ucChar ), ulChar );

				if( ulChar == pdFALSE )
				{
					/* There was no more data to send, so send the stop bit and
					disable interrupts. */
					//I2C_IntCmd( LPC_I2C2, DISABLE );  //aaa-zzz
					NVIC_DisableIRQ(I2C2_IRQn);

			//		I2C_Stop( LPC_I2C2 );  //aaa-zzz   (pending)
		...
		switch( ulI2CStatus )
		{
			case I2C_I2STAT_M_RX_SLAR_ACK:
				...
				if( xBytesOutstanding[ uxI2CNumber ] > 1U )
				{
					...
					LPC_I2C2->CONSET = I2C_I2CONSET_AA;
				}
				else
				{
					/* End the reception after the next byte. */
					//LPC_I2C2->I2CONCLR = I2C_I2CONSET_AA; //aaa-zzz
					LPC_I2C2->CONCLR = I2C_I2CONSET_AA;
			...
			case I2C_I2STAT_M_RX_DAT_NACK:
				...
				ioutilsRX_CHARS_INTO_CIRCULAR_BUFFER_FROM_ISR(
															pxTransferStruct, 		/* The structure that contains the reference to the circular buffer. */
															( ulChar++ == 0UL ), 	/* While loop condition. */
															LPC_I2C2->DAT,		/* Register holding the received character. */
															ulReceived,
															xHigherPriorityTaskWoken
														);
				...
				if( ulI2CStatus == I2C_I2STAT_M_RX_DAT_ACK )
				{
					...
					{
						/* Expecting more than one more byte, keep ACKing. */
						//LPC_I2C2->I2CONSET = I2C_I2CONSET_AA;  //aaa-zzz
						LPC_I2C2->CONSET = I2C_I2CONSET_AA;
					}
					else
					{
